...
import parseSolution, { interpolateColor, normaliseDifficulty } from "@/utils/parseSolution";
...
function Cell({puzzleValue, solutionValue, normalisedDifficulty, hardest = false, easiest = false}: any) {
...
${x}`);
      ...
      py.loadPackage("/roukaour_sudoku_solver.whl").then((c) => {setReady(true)})
    })
      ...
      <textarea value={puzzle} onChange={e => setPuzzle(e.target.value)} className="w-full border mb-4" />
        <div className="grid grid-cols-9 grid-rows-9 text-center mx-auto font-bold">
          {puzzle.split('').map((cell, i) => {
            ...
            return (
              <Cell puzzleValue={cell} solutionValue={solution} hardest={solution === maxDifficulty} easiest={solution === minDifficulty} normalisedDifficulty={(solution?.difficulty && maxDifficulty?.difficulty && minDifficulty?.difficulty) ? normaliseDifficulty(solution?.difficulty, maxDifficulty?.difficulty, minDifficulty?.difficulty):null} key={[puzzle, i].join("")} />
            );
          })}
        </div>
        {lastRun && <>
