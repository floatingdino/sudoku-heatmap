This is a lightly updated version of [roukaour/sudoku](https://github.com/roukaour/sudoku) to allow it to build with Pyodide for use in the browser.

## Building the Pyodide Wheel

Run `make` from the `solver` directory to install dependencies and build the wheel. Alternately run `make install` then `make build`.

(generally implements [this guide](https://pyodide.org/en/stable/development/building-and-testing-packages.html#building-and-testing-packages-out-of-tree))
